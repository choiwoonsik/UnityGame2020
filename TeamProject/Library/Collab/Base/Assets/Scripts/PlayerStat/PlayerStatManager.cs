...
public class PlayerStatManager : MonoBehaviour
{
    ...
    public BuildingManager theBuilding; //건물 보유 체크
    ...
    IEnumerator CheckEstateCoroutine()
    {
        int checkCount = 0;
        playerEstate = 0;
        for(int i = 0; i < theBuilding.myBuildings.Length; i++)
        {
            if (theBuilding.myBuildings[i].buildingSold)
            {
                playerEstate += theBuilding.myBuildings[i].buildingPrice;
                checkCount++;
            }
        }
    ...
    public void PlayerCreditUp()
    {
        ...
        theBuilding.UpdateAllConditions(0);
